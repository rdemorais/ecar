...
<%	try {
		PeriodoExercicioDao perExercicioDao = new PeriodoExercicioDao(request);
		PeriodoExercicioPerExe perExe;
		
		if(session.getAttribute("objExercicio") != null){
			perExe = (PeriodoExercicioPerExe) session.getAttribute("objExercicio");
			session.removeAttribute("objExercicio");
		}else{
			perExe = (PeriodoExercicioPerExe) perExercicioDao.buscar(PeriodoExercicioPerExe.class, Long.valueOf(Pagina.getParamStr(request, "codigo")));
	  	}
...
%>
		...
		<input type="hidden" name="codigo" value="<%=perExe.getCodPerExe()%>">
