...
function validarAlterar(form){
	...
	if(!validaString(form.nomeAre, "Nome", true)){
		return(false);
	}
	if(!validaString(form.indAtivoAre, "Ativo", true)){
		return(false);
	}
	return(true);
}
...
<form name="form" method="post">
	...
	try{
		...
		if(session.getAttribute("objArea") != null){
			area = (AreaAre) session.getAttribute("objArea");
			session.removeAttribute("objArea");
		}else{
			area = (AreaAre) areaDao.buscar(AreaAre.class, Long.valueOf(Pagina.getParamStr(request, "codigo")));
	  	}
		
		_disabled = "";
%>
		<input type="hidden" name="codigo" value="<%=area.getCodAre()%>">
