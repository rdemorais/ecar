<%@ include file="../../login/validaAcesso.jsp"%>

<%@ page import="ecar.pojo.SubAreaSare" %>
<%@ page import="ecar.dao.SubAreaDao" %>
<%@ page import="comum.util.Data" %>
<%@ page import="java.util.List" %>
<%@ include file="../../frm_global.jsp" %>

<html lang="pt-br">
<head>
<%@ include file="../../include/meta.jsp"%>
<%@ include file="/titulo.jsp"%>
</head>
<body>

<form name="form" method="post">
    <!-- campo de controle para a navegação de registros -->
	<input type="hidden" name="hidRegistro" value="<%=Pagina.getParamInt(request, "hidRegistro")%>">
	
	<!-- campo de controle para mensagens -->
	<input type="hidden" name="msgOperacao" value=""> 
	<input type="hidden" name="msgPesquisa" value="">	
	<input type="hidden" name="hidPesquisou" value="">
	
</form>

<%
	SubAreaSare subArea = new SubAreaSare();
	SubAreaDao subAreaDao = new SubAreaDao(request);
	String hidAcao = Pagina.getParam(request, "hidAcao");
	String msg = null;
	String submit = "frm_pes.jsp";

	/* default sempre refazer a pesquisa */
	boolean refazPesquisa = true;  

	boolean incluir = "incluir".equalsIgnoreCase(hidAcao);
	boolean alterar = "alterar".equalsIgnoreCase(hidAcao);
	boolean excluir = "excluir".equalsIgnoreCase(hidAcao);
	boolean pesquisar = "pesquisar".equalsIgnoreCase(hidAcao);

	if (incluir) {
		refazPesquisa = false;
		submit = "frm_inc.jsp";
		try {
			/* melhor usar getParamStr, pois nunca devolve null */		
			if(!"".equals(Pagina.getParamStr(request, "codigoIdentSare")))
				subArea.setCodigoIdentSare(Long.valueOf (Pagina.getParamLong(request, "codigoIdentSare")) );
			subArea.setNomeSare(Pagina.getParamStr(request, "nomeSare").trim()); 
			subArea.setIndAtivoSare(Pagina.getParamStr(request, "indAtivoSare").trim()); 
			subArea.setDataInclusaoSare(Data.getDataAtual());
			
			subAreaDao.salvar(subArea);
			msg = _msg.getMensagem("subArea.inclusao.sucesso");
			session.removeAttribute("objSubArea");
		} catch (ECARException e) {
			org.apache.log4j.Logger.getLogger(this.getClass()).error(e);
			session.setAttribute("objSubArea", subArea);
			msg = _msg.getMensagem(e.getMessageKey());
		} catch (Exception e){
		%>
			<%@ include file="/excecao.jsp"%>
		<%
		}
	} else if (alterar) {
		try {
			/* busca o objeto para manter o que já esta cadastrado */
			subArea = (SubAreaSare) subAreaDao.buscar(SubAreaSare.class, Long.valueOf(Pagina.getParam(request, "codigo")));
			/* altera o que foi modificado na tela */
			if(!"".equals(Pagina.getParamStr(request, "codigoIdentSare")))
				subArea.setCodigoIdentSare(Long.valueOf (Pagina.getParamLong(request, "codigoIdentSare")) );
			else
				subArea.setCodigoIdentSare(null);
			subArea.setNomeSare(Pagina.getParamStr(request, "nomeSare").trim()); 
			subArea.setIndAtivoSare(Pagina.getParamStr(request, "indAtivoSare").trim()); 
			/* altera no BD */
			subAreaDao.alterar(subArea);
			msg = _msg.getMensagem("subArea.alteracao.sucesso");
			session.removeAttribute("objSubArea");
		} catch (ECARException e) {
			org.apache.log4j.Logger.getLogger(this.getClass()).error(e);
			refazPesquisa = false;
			submit = "frm_alt.jsp";
			session.setAttribute("objSubArea", subArea);
			msg = _msg.getMensagem(e.getMessageKey());
		} catch (Exception e){
		%>
			<%@ include file="/excecao.jsp"%>
		<%
		}
	} else if (excluir) {
		try {
			/* busca e exclui o objeto do BD */
			subAreaDao.excluir((SubAreaSare)subAreaDao.buscar(SubAreaSare.class, Long.valueOf(Pagina.getParam(request, "codigo"))));
			msg = _msg.getMensagem("subArea.exclusao.sucesso");
		} catch (ECARException e) {
			org.apache.log4j.Logger.getLogger(this.getClass()).error(e);
			msg = _msg.getMensagem(e.getMessageKey());
		} catch (Exception e){
		%>
			<%@ include file="/excecao.jsp"%>
		<%
		}
	} else if (pesquisar) {
		/* aqui devemos usar getParam, pois queremos null se o campo estiver vazio */
		if(!"".equals(Pagina.getParamStr(request, "codigoIdentSare")))
			subArea.setCodigoIdentSare(Long.valueOf (Pagina.getParamLong(request, "codigoIdentSare")) );
		subArea.setNomeSare((Pagina.getParam(request, "nomeSare"))); 
		subArea.setIndAtivoSare((Pagina.getParam(request, "indAtivoSare"))); 
		
		session.setAttribute("objPesquisa", subArea);
	}
	/* da o alert para incluir, alterar, pesquisar ou excluir */
	if (msg != null)
		Mensagem.setInput(_jspx_page_context, "document.form.msgOperacao.value", msg); 
		
	if (refazPesquisa) {
		try {
			subArea = (SubAreaSare) session.getAttribute("objPesquisa");
			List lista = subAreaDao.pesquisar(subArea, new String[] {"nomeSare","asc"});
			boolean pesquisou = (lista.size() > 0) ? true : false;
			session.setAttribute("lista", lista);
			if (pesquisou) {
				submit = "frm_nav.jsp";
				msg = null;
			} else {
				Mensagem.setInput(_jspx_page_context, "document.form.hidPesquisou.value", "false");
				submit = "frm_pes.jsp";
				msg = _msg.getMensagem("subArea.pesquisa.nenhumRegistro");
			}
		} catch (ECARException e) {
			org.apache.log4j.Logger.getLogger(this.getClass()).error(e);
			msg = _msg.getMensagem(e.getMessageKey());
		} catch (Exception e){
		%>
			<%@ include file="/excecao.jsp"%>
		<%
		}
		if (msg != null)
			Mensagem.setInput(_jspx_page_context, "document.form.msgPesquisa.value", msg);
	}
	%>
	<script language="javascript">
		document.form.action = "<%=submit%>";
		document.form.submit();
	</script> 


</body>
</html>


