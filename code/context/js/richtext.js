...
//		document.writeln('<br /><input type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\',' + buttons + ');" />&nbsp;<label for="chkSrc' + rte + '">View Source</label>');
...
function setHiddenVal(rte) {
	...
	if(oHdnField != null){
		...
		if (stripHTML(oHdnField.value.replace("&nbsp;", " ")) == "" &&
			...
			if (document.getElementById('richText'))
				document.getElementById('richText').value = oHdnField.value;
			else
				form.richText.value = oHdnField.value;
	}
	//AQUI	
	//return oHdnField.value;
}

function setHiddenParecerVal(rte) {
	//set hidden form field value for current rte
	var oHdnField = document.getElementById('hdn' + rte);
	var is_chrome = navigator.userAgent.toLowerCase().indexOf('chrome') > -1;
	
	if (is_chrome) {
	    oHdnField = document.getElementById(rte);
	}
	...
	if (stripHTML(oHdnField.value.replace("&nbsp;", " ")) == "" &&
		...
		if (document.getElementById('richText'+rte))
			document.getElementById('richText'+rte).value = oHdnField.value;
		//else
		//	form.richText+rte.value = oHdnField.value;
...
function toggleHTMLSrc(rte, buttons) {
	...
	var oHdnField = document.getElementById('hdn' + rte);
	if (document.getElementById("chkSrc" + rte)!=null && document.getElementById("chkSrc" + rte).checked) {
...
function toggleHTMLSrcModified(rte, buttons) {
...
function dlgColorPalette(rte, command) {
	...
	var iLeftPos = getOffsetLeft(buttonElement) - (oDialog.offsetWidth - buttonElement.offsetWidth);
...
function dlgInsertTable(rte, command) {
	...
	parent.command = command;
	currentRTE = rte;
	InsertTable = popUpWin(includesPath + '/rte/insert_table.htm', 'InsertTable', 360, 210, '');
}
...
function dlgInsertLink(rte, command) {
	...
	parent.command = command;
	currentRTE = rte;
	InsertLink = popUpWin(includesPath + '/rte/insert_link.htm', 'InsertLink', 360, 180, '');
...
function desabilitaCkeditor(rte) {
