...
public class RelatorioPPALinhaAcaoServlet extends RelatorioServlet {
	...
	private Logger logger = Logger.getLogger(this.getClass());
		...
		logger.info("Generating relatorio PPA Por Linha de Ação...");
...
//		byte[] bytes = null;
		...
		try {
			...
			if (possuiGerente!=null && "true".equalsIgnoreCase(possuiGerente) ){
				parametros.put("coluna1", "Linha de Ação / Programa / Gerente" );
			}else{
				parametros.put("coluna1", "Linha de Ação / Programa" );
			}
			...
			logger.error( "Nao foi possivel gerar relatorio PPA por Linha de Ação",e);
		}
