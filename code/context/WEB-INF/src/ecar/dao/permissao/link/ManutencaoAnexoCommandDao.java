 ...
 * @author 70744416353
 */
public class ManutencaoAnexoCommandDao extends Dao implements PermissaoAcessoLinkCommand{
	 ...
	 */
	 public String execute(String evento, String parametros, SegurancaECAR seguranca, ServletContext servletContext) throws ECARException {
		...
		try {
			...
			url = URLEvento.montaURLRedirecionamento(Dominios.CFG_MAIL_MANUTENCAO_ANEXO.toString(), parametros, servletContext);
		...
		public void permissaoAcesso(String parametros, SegurancaECAR seguranca) throws PermissaoAcessoLinkException {
			...
			for(int i=0; i< valores.length; i++) {
				...
				if(valores[i].contains("codIettuc")) {
					
					posicao =	valores[i].indexOf("=");
					String nomeParametro = valores[i];
					codIettuc= valores[i].substring(posicao+1, (valores[i].length()));
				...
				} else if(valores[i].contains("codIett")) {
