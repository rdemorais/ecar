	...
	private boolean checkJpeg() throws IOException {
		...
		while (true) {
			...
			if (collectComments && size > 2 && marker == 0xfffe) { // comment
				...
				String comment = new String(chars, Dominios.ENCODING_DEFAULT);
...
/*	private String readLine(int firstChar) throws IOException {
	...
	private void skip(int num) throws IOException {
		...
		larguraImagem = this.getWidth();		
		alturaImagem = this.getHeight();
		
		Integer largura = Integer.valueOf(larguraImagem);
		Integer altura = Integer.valueOf(alturaImagem);
		Integer limiteMaximo = Integer.valueOf(limite);
		
		if (larguraImagem > 0 && alturaImagem > 0) { 
			
			if (larguraImagem < limite && alturaImagem > limite) {
				
				proporcao = altura.doubleValue() / limiteMaximo.doubleValue();
				conversao = largura.doubleValue() / proporcao;	
				
				BigDecimal bd = new BigDecimal(conversao);
				bd = bd.setScale(decimalPlace,BigDecimal.ROUND_UP);
				conversao = bd.doubleValue();   
				
				larguraImagem = (int) conversao;
				alturaImagem = limite;						
			...
			if (larguraImagem > limite && alturaImagem < limite) {
				...
				proporcao =  largura.doubleValue() / limiteMaximo.doubleValue();
				conversao = altura.doubleValue() / proporcao;	
				
				BigDecimal bd = new BigDecimal(conversao);
				bd = bd.setScale(decimalPlace,BigDecimal.ROUND_UP);
				conversao = bd.doubleValue();    
				
				alturaImagem = (int) conversao;
				larguraImagem = limite;				
				
			}	
			
			if (larguraImagem > limite && alturaImagem > limite) {

				diferenca1 = larguraImagem - limite;
				
				diferenca2 = alturaImagem - limite;
		...
		this.width = larguraImagem;
		this.height = alturaImagem;
         ...
         * @param zoom
         */
        public void zoomImagem(String zoom){
		
		int larguraImagem;		
	    int alturaImagem;   			
	    double zoomAltura = 0;	    
	   	double zoomLargura = 0;	   	
	   	int decimalPlace = 0;
		...
		if ("50%".equals(zoom)){
			zoomAltura = altura.doubleValue() * 0.5;
			zoomLargura = largura.doubleValue() * 0.5;	
		}
			
		if ("75%".equals(zoom)){
			zoomAltura = altura.doubleValue() * 0.75;
			zoomLargura = largura.doubleValue() * 0.75;	
		}
		
		if ("150%".equals(zoom)){
			zoomAltura = altura.doubleValue() * 1.5;
			zoomLargura = largura.doubleValue() * 1.5;	
		}
		
		if ("200%".equals(zoom)){
			zoomAltura = altura.doubleValue() * 2;
			zoomLargura = largura.doubleValue() * 2;	
		}
		
		if ("300%".equals(zoom)){
			zoomAltura = altura.doubleValue() * 3;
			zoomLargura = largura.doubleValue() * 3;	
		}
		
		BigDecimal bdAltura = new BigDecimal(zoomAltura);
		bdAltura = bdAltura.setScale(decimalPlace,BigDecimal.ROUND_UP);
		zoomAltura = bdAltura.doubleValue();
			
		BigDecimal bdLargura = new BigDecimal(zoomLargura);
		bdLargura = bdLargura.setScale(decimalPlace,BigDecimal.ROUND_UP);
		zoomLargura = bdLargura.doubleValue();
			
		alturaImagem = (int) zoomAltura;
		larguraImagem = (int) zoomLargura;	
