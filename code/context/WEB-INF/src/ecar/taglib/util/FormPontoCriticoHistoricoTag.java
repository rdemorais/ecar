...
/**
 * As referencias para AtributoEstruturaTela foram subtituidas por
 * ObjetoEstrutura.<br>
 * 
 * @author
 */
public class FormPontoCriticoHistoricoTag implements Tag {
	
	private ValidaPermissao validaPermissao = new ValidaPermissao();
	private ObjetoEstrutura atributo;
	private HistoricoPontoCriticoPtc pontoCriticoPtc;
		...
		try {
			...
			if (atributo.iGetTipo() == EstruturaAtributoEttat.class) {
				...
				if (atributo.iGetIndOpcional() == null || atributo.iGetIndOpcional().booleanValue() == false) {
					// Mantis 6514: para atributos não opcionais verificar pelo
					// campo "sequencia de apresentacao em telas de informação"
	...
	public void geraHTMLIndAtivoPtc() {
		...
		try {
			criaRadio(atributo.iGetNome()+codigo, atributo.iGetLabel(), atributo.iGetValor(getPontoCriticoPtc()), opcoes, atributo.iGetDica());
		}
	...
	public void geraHTMLDataInclusaoPtc() {
		try {
			if (atributo.iGetIndOpcional() == null || atributo.iGetIndOpcional().booleanValue() == false) {
				if (atributo.iGetSequenciaCampoEmTela() != null && atributo.iGetSequenciaCampoEmTela().intValue() != 0) {
					criaLabelText(atributo.iGetNome()+codigo, atributo.iGetLabel(), "15", "15", atributo.iGetValor(getPontoCriticoPtc()), atributo.iGetDica());
				}
			}
			else {
	...
	public void geraHTMLDataSolucaoPtc() {
		try {
			criaInputTextData(atributo.iGetNome()+codigo, atributo.iGetLabel(), atributo.iGetValor(getPontoCriticoPtc()), atributo.iGetDica());
		}
		catch (ECARException e) {
			Logger logger = Logger.getLogger(this.getClass());
			logger.error(e);
	...
	public void geraHTMLDescricaoSolucaoPtc() {
		try {
			if (atributo.iGetTamanhoConteudoAtrib() > ObjetoEstrutura.DEFAULT_TAMANHO_CAMPO_TEXT) {
				criaTextArea(atributo.iGetNome()+codigo, atributo.iGetLabel(), "4", "60", atributo.iGetValor(getPontoCriticoPtc()), atributo.iGetDica());
			}
			else {
				criaInputText(atributo.iGetNome()+codigo, atributo.iGetLabel(), atributo.iGetTamanhoConteudoAtrib().toString(), atributo.iGetTamanhoConteudoAtrib()
				        .toString(), atributo.iGetValor(getPontoCriticoPtc()), atributo.iGetDica());
			}
		}
		catch (ECARException e) {
			Logger logger = Logger.getLogger(this.getClass());
	...
	public void geraHTMLUsuarioUsu() {
		try {
			criaPesquisa(atributo.iGetNome()+codigo, true, atributo.iGetLabel(), "ecar.popup.PopUpUsuario", "50", atributo.iGetValorCodFk(getPontoCriticoPtc()),
			        atributo.iGetValor(getPontoCriticoPtc()), "100", atributo.iGetDica());
		}
		catch (ECARException e) {
			Logger logger = Logger.getLogger(this.getClass());
			logger.error(e);
	...
	public void geraHTMLUsuarioUsuInclusao() {
		try {
			if (atributo.iGetIndOpcional() == null || atributo.iGetIndOpcional().booleanValue() == false) {
				if (atributo.iGetSequenciaCampoEmTela() != null && atributo.iGetSequenciaCampoEmTela().intValue() != 0) {
					criaLabelText(atributo.iGetNome()+codigo, atributo.iGetLabel(), "50", "50", atributo.iGetValor(getPontoCriticoPtc()), atributo.iGetDica());
				}
			}
			else {
				if (atributo.iGetObrigatorio() != null && atributo.iGetObrigatorio().booleanValue() == true) {
	...
	public void geraHTMLPontoCriticoCorPtccores() throws IOException, NoSuchAlgorithmException {
		...
		try {
			s.append("<tr>");
			s.append("	<td class=\"label\">" + atributo.iGetLabel() + "</td>");
			s.append("	<td>&nbsp;</td>");
			s.append("</tr>");
			s.append("	<tr>");
			s.append("		<td>&nbsp;</td>");
			...
			while (itCores.hasNext()) {
				...
				if (cor.getIndPontoCriticoCor().equals("S")) {
					...
					if (imagePath != null) {
						...
						/*
						 * -- As tags do CSS "max-width" e "max-height" não
...
//					}
	...
	public void geraHTMLAcompRelatorioArel() throws IOException {
		...
		StringBuffer s = new StringBuffer();
		/*
		 * Verificar o grupo de acesso que esse usuario faz parte e quais as
		 * funções de acompanhamento que ele pode representar Como a funcao de
		 * acompanhamento é ligada ao relatorio. O código passado é o codigoArel
		 * (codigoigo d o relatorio em que o usuario está dando parecer) mas o
		 * texto exibido é o label do Tipo funcao de acompanhamento.
		 */
		writer.print(s.toString());
	}
	 ...
	 * @version 0.2, 22/03/2007
         ...
         * @param maxlength
	 */
	public void criaPesquisa(String nome, boolean IndAtivoUsu, String label, String classePesquisa, String size, String valor, String valorText,
		...
		try {
			...
			s.append(label);
			s.append("</TD>").append("<TD>").append("<input type=\"text\" disabled name=\"nome").append(nome).append("\" size=\"").append(size).append(
			        "\" value=\"").append(valorText).append("\" maxlength=\"").append(maxlength).append("\"").append(">").append(imagem_inativa).append(
			        "<input type=\"hidden\" name=\"").append(nome).append("\" value=\"").append(valor).append("\">");
			// Adiciona o campo
			if (this.getExibirBotoes().booleanValue()) {
				s.append("&nbsp;&nbsp;<input type=\"button\" name=\"pesq\" value=\"Pesquisar\" class=\"botao\" ");
				if (getBloquearCampo())
					s.append(" disabled");
				s.append(" onclick=\"popup_pesquisa('").append(classePesquisa).append("', 'retorno").append(nome).append("');\">").append(
				        "&nbsp;&nbsp;<input type=\"button\" name=\"pesq\" value=\"Limpar\" class=\"botao\" ").append("onclick=\"document.form.nome").append(
				        nome).append(".value=''; document.form.").append(nome).append(".value=''\"");
				if (getBloquearCampo())
					s.append(" disabled");
				s.append(">");
			}
	...
	public void criaRadio(String nome, String label, String valor, Collection opcoes, String dica) {
		...
		try {
			...
			if (opcoes != null) {
				...
				while (it.hasNext()) {
					String[] chaveValor = (String[]) it.next();
					s.append("<input type=\"radio\" class=\"form_check_radio\" name=\"").append(nome).append("\" value=\"").append(chaveValor[0]).append("\"");
					if (getBloquearCampo())
						s.append(" disabled");
					if (chaveValor[0].equals(valor))
						s.append(" checked ");
				...
				s.append("&nbsp;<input type=\"button\" name=\"buttonLimpar\" value=\"Limpar\" class=\"botao\" ").append(
				        "onclick=\"limparRadioButtons(document.getElementsByName('" + nome + "'));\"");
				if (getBloquearCampo())
					s.append(" disabled ");
				s.append(">");
			}
	...
	public void criaInputButton(String label, String name, String onclick) {
		...
		try {
			...
			/*
			 * Regra de bloqueio destes botï¿½es ï¿½ diferente. Deve ficar
			 * sempre liberado, menos nos casos que o usuï¿½rio nï¿½o tem
			 * permissï¿½o.
			 */
			if (getDesabilitar() != null && getDesabilitar()) {
				s.append(" disabled ");
			}
			s.append("value=\"").append(label).append("\" name=\"bt").append(name).append("\" onclick=\"").append(onclick).append("\"").append("></TD></TR>");
			writer.print(s.toString());
		}
		catch (IOException e) {
			Logger logger = Logger.getLogger(this.getClass());
	...
	public void criaInputText(String nome, String label, String size, String maxlength, String valor, String dica) {
		...
		try {
			...
			// adiciona o texto
			s.append(label);
			s.append("</TD>").append("<TD nowrap>").append("<input type=\"text\" name=\"").append(nome).append("\" size=\"").append(size).append("\" value=\"")
			        .append(valor).append("\" maxlength=\"").append(maxlength).append("\"");
			if (getBloquearCampo())
				s.append(" disabled");
			s.append(">");
			// Aplica a dica de uso da linha, caso ela exista no cadastro.
	...
	/**
	 * Cria uma linha no formulï¿½rio com campo nï¿½o editï¿½vel para
	 * exibiï¿½ï¿½o de textos. <br>
	 * 
	 * @author n/c, rogerio
	 * @since 0.1, n/c
	 * @version 0.2, 22/03/2007
	...
	/**
	 * Cria uma linha no formulï¿½rio para um campo text para receber dados no
	 * formato de moeda. <br>
	 * 
	 * @author n/c, rogerio
	 * @since 0.1, n/c
	...
	public void criaInputTextMoeda(String nome, String label, String size, String maxlength, String valor, String dica) {
		...
		try {
			 ...
			 */
			s.append("<input type=\"text\" name=\"").append(nome).append("\" size=\"").append(size).append("\" value=\"").append(valor).append(
			        "\" maxlength=\"").append(maxlength).append("\"");
			if (getBloquearCampo())
				s.append(" disabled");
			s.append(">");
			// Aplica a dica de uso da linha, caso ela exista no cadastro.
	...
	public void criaInputTextData(String nome, String label, String valor, String dica) {
		...
		try {
			...
			if (!this.getBloquearCampo() && nome != null && nome.startsWith(Dominios.PREFIX_DATA)) {
				s
				        .append("		<img class=\"posicao\" title=\"Selecione a data\" src=\"../../images/icone_calendar.gif\" onclick=\"open_calendar('data', document.forms[0]."
				                + nome + ", '')\" " + this.getBloquearCampo() + " " + this.getBloquearCampo() + ">");
			}
			s.append("</TD></TR>");
			writer.print(s.toString());
		}
	...
	/**
	 * Cria linha no formulï¿½rio com campo Text Area para entrada de longos
	 * textos.<br>
	 * 
	 * @author n/c, rogerio
	 * @since 0.1, n/c
	...
	public void criaTextArea(String nome, String label, String rows, String cols, String valor, String dica) {
		...
		try {
			...
			s.append("<TD>").append("<div style=\"float: left;\">").append("<textarea name=\"").append(nome).append("\" rows=\"").append(rows).append(
			        "\" cols=\"").append(cols);
			String tam = "2000";
			if (atributo.iGetTamanhoConteudoAtrib() != null) {
			...
			s.append(">").append(valor).append("</textarea>").append("</div><div style=\"float: left;\"><br><br>");
			// Aplica a dica de uso da linha, caso ela exista no cadastro.
			// Mantis #8688. Por Rogï¿½rio (22/03/2007)
			if (dica != null && !"".equals(dica))
				s.append(Util.getTagDica(nome, this.getContextPath(), dica));
			...
			for (int i = 0; i < areas.size(); i++) {
				...
				for (int j = 0; j < lSubAreas.size(); j++) {
					...
					if ("S".equals(subArea.getIndAtivoSare()))
						s.append("aSubArea[").append(i + 1).append("][").append(indiceSubArea++).append("] = new Option('").append(subArea.getNomeSare())
						        .append("','").append(subArea.getCodSare().toString()).append("');\n");
				}
			}
			s.append("</script>\n");
			writer.print(s.toString());
	...
	public void criaDiv(String nome, String label, String dica) {
		...
		try {
			...
			if (atributo.iGetObrigatorio() != null && atributo.iGetObrigatorio().booleanValue() == true)
				s.append("* ");
			s.append(label).append("</TD>");
			s.append("<TD>").append("<div id=\"").append(nome).append("\" style=\"float: left;\"></div>").append("<div style=\"float: left;\">");
			// Aplica a dica de uso da linha, caso ela exista no cadastro.
			// Mantis #8688. Por Rogï¿½rio (22/03/2007)
			if (dica != null && !"".equals(dica))
				s.append(Util.getTagDica(nome, this.getContextPath(), dica));
	 ...
	 */
	public HistoricoPontoCriticoPtc getPontoCriticoPtc() {
	 ...
	 */
	public void setItemEstrutura(HistoricoPontoCriticoPtc pontoCriticoPtc) {
	...
	public Boolean getBloquearCampo() {
		// Primeiro verifica se a interface ï¿½ de consulta. neste caso, deve
		// aparecer bloqueado.
		if (getDesabilitar() != null && getDesabilitar()) {
			return true;
		}
		// caso a interface nï¿½o seja de consulta, deve verificar se o
		// planejamento esteja bloqueado.
		if (getIett().getIndBloqPlanejamentoIett() != null && "S".equals(getIett().getIndBloqPlanejamentoIett())) {
			// Se o planejamento estï¿½ bloqueado, ï¿½ verificado a
			// configuraï¿½ï¿½o do atributo para
			// ver se ele pode ser editado mesmo com o planejamento bloqueado.
			if (atributo.iGetBloqueado()) {
				// ItemEstruturaDao itemEstruturaDao =
				// pontoCriticoPtc.getItemEstruturaIett();
				return true; // quando o campo estï¿½ liberado retorna falso
			}
		}
         ...
         * @param pontoCriticoPtc
         */
        public void setPontoCriticoPtc(HistoricoPontoCriticoPtc pontoCriticoPtc) {
		this.pontoCriticoPtc = pontoCriticoPtc;
	}
