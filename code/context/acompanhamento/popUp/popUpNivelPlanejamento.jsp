...
function continuar(){
	...
	var algumNivelCheckado = false;
	var semInformacaoCheckado = false;
	
	for(i = 0; i< document.form.nivel.length;i++){
		
		if (document.form.nivel[i].checked){
			algumNivelCheckado = true;
			break;
		}					
	}

	if (document.form.semInformacaoNivel.checked){
		semInformacaoCheckado = true;
	}

	if (algumNivelCheckado || semInformacaoCheckado){
		...
		if(diferentes == 0 && window.opener.document.forms[0].semInformacaoNivelPlanejamento.value == "<%= hidOpcaoSemInformacao%>"){
			...
			window.opener.document.forms[0].niveisPlanejamentoAlterado.value = "S";
			
			<%
			if(!"S".equals(hidTelaGerarPeriodo)){
			%>
				window.opener.reloadNivelPlanejamento();
			<%
			}
			%>
		...
		alert('Selecione pelo menos uma opção de filtro');
	}
		...
		<form name="form" method="post" action="">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<%
						...
						// funcionamento: 1. obter o grupo de atributos de nivel de planejamento 
						//                2. obter a lista de atributos ordenada desse grupo;	
				
				// obtem todos os grupos de atributo de nivel de planejamento
				List niveisPlanejamento = new SisGrupoAtributoDao().getAtributosOrdenados(configuracaoCfg.getSisGrupoAtributoSgaByCodSgaGrAtrNvPlan());
					
				Iterator it = niveisPlanejamento.iterator();
				while (it.hasNext()) {
					SisAtributoSatb nivel = (SisAtributoSatb) it.next();
					if(nivel.getIndAtivoSatb().equals("S")) {
				%>
				<tr>
					<td align="right">
