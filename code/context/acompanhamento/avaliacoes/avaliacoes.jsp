...
try{ 
	...
	StatusRelatorioSrl statusLiberado = (StatusRelatorioSrl) ariDao.
                                            buscar(StatusRelatorioSrl.class, Long.valueOf(AcompReferenciaItemDao.STATUS_LIBERADO));

	TipoAcompanhamentoTa ta = (TipoAcompanhamentoTa) tipoAcompDao.buscar(TipoAcompanhamentoTa.class, Long.valueOf(Pagina.getParamStr(request, "codTipoAcompanhamento")));
	
	String exigeLiberarAcompanhamento = ta.getIndLiberarAcompTa();
...
<div id="conteudo"> 
	...
	<%--
	/* abaSuperior ="S"  atributo para as abs que
	 contém agrupamentos de informações do conjunto
	  de acompanhamentos --%>
	<util:barraLinksRegAcomp
		acompReferenciaItem="<%=ari%>"  
		selectedFuncao="AVALIACOES"
	...
	if(ta.getEstruturaEtt() != null && ari.getItemEstruturaIett() != null && ari.getItemEstruturaIett().getItemEstruturaIett() != null){
		telaAnt = _pathEcar + "/acompanhamento/proximosItens.jsp?itemDoNivelClicado=" + ari.getItemEstruturaIett().getItemEstruturaIett().getCodIett() + "&codTipoAcompanhamento=" + Pagina.getParamStr(request, "codTipoAcompanhamento") + "&hidFormaVisualizacaoEscolhida="+Pagina.getParamStr(request, "hidFormaVisualizacaoEscolhida")+ "#ancora" + ari.getItemEstruturaIett().getCodIett();
	}
	else{
		telaAnt = _pathEcar + "/acompanhamento/posicaoGeral.jsp?fecharMenu=true&titulo=Geral&codTipoAcompanhamento=" + Pagina.getParamStr(request, "codTipoAcompanhamento") + "&periodo=" + Pagina.getParamStr(request, "periodo") + "&hidFormaVisualizacaoEscolhida="+Pagina.getParamStr(request, "hidFormaVisualizacaoEscolhida")+ "#ancora" + ari.getItemEstruturaIett().getCodIett();
	}
...
<%
	...
	if("N".equals(exigeLiberarAcompanhamento) || ari.getAcompRelatorioArels() != null){
		lista = ariDao.getAcompRelatorioArelOrderByFuncaoAcomp(ari);
	}
...
%>
		...
		<!--  3 Ultimas Posicoes... -->
		<%
		List arels = ariDao.getUltimosAcompanhamentosItem(acompRelatorio.getAcompReferenciaItemAri(), acompRelatorio.getTipoFuncAcompTpfa(), Integer.valueOf(3));
