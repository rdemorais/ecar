...
function setarValoresSentinela(codUsu, nomeUsu, login, cpfCnpjUsu, email){
	str =  '?nomeUsu='+nomeUsu;
	str += '&idUsuarioUsu='+login;
	str += '&idDominioUsu='+codUsu;
	str += '&cnpjCpfUsu='+cpfCnpjUsu;
	str += '&email1Usu='+ email;
	
	document.forms[0].action = '../usuarios/frm_inc.jsp'+str;
...
function validarGravar(form){
	if (form.vinculandos  && form.naoVinculandos) {
...
<form name="form" method="post">
	...
	if(session.getAttribute("objUsuario") != null){
		...
		usuario.setIdUsuarioUsu(Pagina.getParamStr(request, "idUsuarioUsu"));
 	}
