...
<table cellspacing="0" cellpadding="0">
	<tr>
		<td class="conteudo_interno">
			...
			<div class="imagemartigo_tres_esquerda_sem_tamanho">
			...
			<div class="imagemartigo_tres_direita_sem_tamanho">
				<% if (item.getImagem3Sgti() != null && !"".equals(item.getImagem3Sgti().trim())){ 
				
						imagem3Caminho = pathRaiz + item.getImagem3Sgti();
					   	ImageInfo imagem3 = new ImageInfo();
					   	File arquivo3 = new File(imagem3Caminho);
					   	
					   	String hashNomeArquivo = null;
						UsuarioUsu usuarioImagem = null;
					   	
					   	hashNomeArquivo = Util.calcularHashNomeArquivo(pathRaiz, item.getImagem3Sgti());
						usuarioImagem = ((ecar.login.SegurancaECAR)request.getSession().getAttribute("seguranca")).getUsuario();  
						Util.adicionarMapArquivosAtuaisUsuarios(usuarioImagem, hashNomeArquivo, pathRaiz, item.getImagem3Sgti());
					   	
					   	if (arquivo3.exists()){
					   	
    				    	FileInputStream file3 = new FileInputStream(imagem3Caminho);
    				    	imagem3.setInput(file3);
				...
				<%  		} 
    							...
    							imagem3.redimensionaImagem(limite);
    							larguraImagem3 = imagem3.getWidth();		
								alturaImagem3 = imagem3.getHeight();	 
    			%>
				<img src="<%=request.getContextPath()%>/DownloadFile?tipo=open&RemoteFile=<%=hashNomeArquivo%>" width="<%=larguraImagem3%>" height="<%=alturaImagem3%>">
				<%			}
						}
				...
				<%
						...
						if (item.getLegendaImag3Sgti() != null){
				%>
				<p class="legenda"><%=item.getLegendaImag3Sgti()%></p>
				<%
						}	
					} 
				%>
		 	</div></p><p><%=item.getIntegraSgti()%>	</p>					
