...
function subCapaEmail(email){
	document.form.hidAcao.value = "EMAIL";
	document.form.action = "./email/caixa_entrada.jsp";
...
<form name="form" method="post" action="ctrl_artigos.jsp"><input
	type="hidden" name="hidAcao" value=""> <input type="hidden"
	name="codSgti" value=""> <input type="hidden" name="codAgeo"
	value=""> <input type="hidden" name="codEmail" value="" /></form>
<br>
<table id="conteudo_index" cellspacing="0">
	<tr>
		<td class="destaque1">
			<h1>MEU SISTEMA</h1>
			<p>Bem vindo 
				<b>	<%=usuarioLogado.getNomeUsuSent()%>.</b>
				<br> Seu último acesso foi em <info:lastLogin />
			</p>
			<h2>Meu Cadastro</h2>
			...
			<h2>Caixa de Entrada</h2>
				...
				<%
					...
					email.setUsuarioUsu(usuarioLogado);
					...
					if(lista != null)
						if(lista.size() == 0)
							out.print(" a caixa de e-mail est&aacute; vazia");
						
						else{
							%>
							<a href="#" onclick="javascript:subCapaEmail(0);"><p>Você tem <%=quantidadeEmails%> <br> E-mails não lidos</p></a>
							<% 
					}
				%>
			
		</td>
		<td class="destaque2" colspan="2">
		<%
			SecaoBean secaoSuperDestaque = capaPrincipal.getSuperDestaqueArtigos();

			if (secaoSuperDestaque != null && (secaoSuperDestaque.getInformacaoBean() != null && secaoSuperDestaque.getInformacaoBean().size() > 0)) {
		...
		%>
		<div class="imagem_noticia_home_sem_tamanho"><a
			href="javascript:detalhe('<%=infoSuperDestaque.getLink()%>', <%=infoSuperDestaque.getCodSgti().intValue()%>);">
		<%
			if (!imagem.check()) {
 ...
 %>
		...
		<%=infoSuperDestaque.getFonte().toUpperCase()%> <br>
		<br>
		...
		<%
			...
			if (secaoAgenda != null && (secaoAgenda.getInformacaoBean() != null && secaoAgenda.getInformacaoBean().size() > 0)) {
				...
				while (iterator.hasNext()) {
					...
					if (cont < 6) {
		%> <a
			href="javascript:detalhe('<%=infoAgenda.getLink()%>', <%=infoAgenda.getCodSgti().intValue()%>);">
 ...
 %>
		...
		%> <a href="#" onclick="javascript:subCapaAgenda();" class="iconmais"><img
			src="../images/icon_mais.png" border="0"></a></td>
		<td class="destaque2">
		<h1>MAIS ARTIGOS</h1>
		<%
		...
		%>
		<p><b><a
			href="javascript:detalhe('<%=infoMaisArtigos.getLink()%>', <%=infoMaisArtigos.getCodSgti()%>);"><%=infoMaisArtigos.getTitulo()%></a></b>
		<br>
		(<%=infoMaisArtigos.getDataHora()%>)<br>
		<a
			href="javascript:detalhe('<%=infoMaisArtigos.getLink()%>', <%=infoMaisArtigos.getCodSgti()%>);"><%=infoMaisArtigos.getDescricao()%></a></p>
		<%
			}
		...
		%> <a href="#" onclick="javascript:subCapaArtigos();"
			class="iconmais"><img src="../images/icon_mais.png" border="0"></a>
		</td>
		<td class="destaque2">
		...
		<%
			...
			if (secaoTaxacaoClipping != null && (secaoTaxacaoClipping.getInformacaoBean() != null && secaoTaxacaoClipping.getInformacaoBean().size() > 0)) {
				...
				while (iterator.hasNext()) {
					InformacaoBean infoMaisArtigos = (InformacaoBean) iterator.next();
		%> <a
			href="javascript:detalhe('<%=infoMaisArtigos.getLink()%>', <%=infoMaisArtigos.getCodSgti().intValue()%>);">
		<p><b>(<%=infoMaisArtigos.getDataHora()%>)</b> <%=infoMaisArtigos.getDescricao()%><br>
		<br><%=infoMaisArtigos.getTitulo()%></p>
		</a> <%
 	}
 ...
 %>
		...
		%> <a href="#" onclick="javascript:subCapaTaxacao();"
			class="iconmais"><img src="../images/icon_mais.png" border="0"></a>
		</td>
	</tr>

	<%
		DestaqueAreaDtqa destaqueArea = (DestaqueAreaDtqa) new DestaqueAreaDao(request).buscar(DestaqueAreaDtqa.class, Long.valueOf("1"));
	...
	<tr>
		...
		%>
		<td>
		<h1><%=subDestaque.getIdentificacaoDtqsa()%></h1>
		<%
		...
		%> <a
			href="javascript:detalhe('ctrl_agenda.jsp', <%=item.getAgendaOcorrenciaAgeo().getCodAgeo()%>);">
		...
		</a> <a href="#" onclick="javascript:subCapaAgenda();" class="iconmais"><img
			src="../images/icon_mais.png" border="0"></a> <%
 ...
 %> </a></p>
		<a href="#" onclick="javascript:subCapaGenerica('<%=linkSgtl%>');"
			class="iconmais"><img src="../images/icon_mais.png" border="0"></a>

		<%
			} else {
		%> <br>
		<a href="<%=linkSgtil%>" class="iconmais"><img
			src="../images/icon_mais.png" border="0"></a> <%
 	}
 					}
 				}
...
<%
	...
	if(getCookie("estadoMenu")!=null){
		...
		mudaEstado(getCookie("estadoMenu"));
	}else{
		botao('aberto');
		mudaEstado('aberto');
	}
