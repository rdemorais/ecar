...
<jsp:directive.page import="ecar.popup.PopUpUsuario"/><%@ taglib uri="/WEB-INF/ecar-util.tld" prefix="util" %>
...
<%@page import="ecar.popup.PopUpRegDemanda"%><html lang="pt-br">
<head>
...
<form name="form" method="post" action="popup_pesquisa.jsp?hidAcao=navegar" onsubmit="aoClicarPesquisar(document.form);">
...
<%
	try { 
		...
		if (p instanceof PopUpRegDemanda) {
			p = new PopUpRegDemanda(request);
		}
		...
		boolean buscaPadrao = false;
 		...
 		//pega as visões que o usuário tem acesso
		VisaoDao visaoDao = new VisaoDao(request);		
		List visoesAcesso = visaoDao.getVisoesGrupoAcesso(seguranca.getUsuario(), false, request);
		...
		if ("pesquisar".equalsIgnoreCase(hidAcao) || buscaPadrao) {
			...
			session.setAttribute("visoesAcesso", visoesAcesso);
