...
function adicionar(form, codLit, identificacaoLit){
	...
	if (verificaRadios(form, "hidOpcoes") > 1) {
		for (i = 0; i < form.hidOpcoes.length; i++) {
			...
			if (window.opener.adicionaFilho(dados[0], dados[1]))
				inserido = true;
			selecionado = true;
			break;
			}
...
<form name="form" method="post" action="popup_pesquisa_filho.jsp?hidAcao=navegar" onsubmit="aoClicarPesquisarFilho(document.form);">
		...
		<table class="form">
			<tr>
				...
				<td>
					...
					<input type="button" name="btnOk" class="botao" value="Ok" onclick="aoClicarPesquisarFilho(document.form);">
				</td>
			</tr>
			<tr>
...
<%
		...
		for (int i = hidNumPagina*ITENS_PAGINA; i < limite; i++) {
			...
			String valorFilho = p.getCodigo() + ":" + p.getDescricao();
			if(p instanceof ecar.popup.PopUpUsuarioEGrupo) {
				...
				valorFilho + "\"></td><td class=\"" + classRadio + "\">" +  p.getDescricao());
				...
				out.println("<tr><td><input type=\"radio\" class=\"form_check_radio\" name=\"hidOpcoes\" value=\"" + 
				valorFilho + "\"></td><td class=\"texto\">" +  p.getDescricao());
			}
